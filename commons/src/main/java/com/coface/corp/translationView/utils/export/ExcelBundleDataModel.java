package com.coface.corp.translationView.utils.export;
...
public class ExcelBundleDataModel
{
  ...
  public void feedWithLocalizedTranslations(final String applicationName, final String locale, final List<Translation> localizedTranslations)
  {
    ...
    {
      if (bundleData.get(localizedTranslation.getId().getTagId()) == null)
      {
        bundleData.put(localizedTranslation.getId().getTagId(), new ArrayList<String>());
      }
      int indexForTranslation = this.getIndexForTranslation(locale);
      bundleData.get(localizedTranslation.getId().getTagId()).add(indexForTranslation, localizedTranslation.getValue());
    }
  }
